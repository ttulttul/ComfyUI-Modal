...
class R2StorageConfiguration(StorageBackingConfigurationNode):
    """Declare a Cloudflare R2 backing cache authorized by the local controller."""

    @classmethod
    def define_schema(cls) -> io.Schema:
        """Expose portable R2 metadata while keeping credentials out of workflows."""
        return io.Schema(
            node_id=R2_STORAGE_CONFIGURATION_NODE_ID,
            display_name="R2 Storage Configuration",
            category="Remote Execution/Configuration/Storage",
            description=(
                "Provide a shared Cloudflare R2 backing cache to Vast.ai and SSH "
                "targets. Use Login on the node to authorize Cloudflare without "
                "executing the workflow."
            ),
            inputs=_r2_storage_inputs(),
    ...
    def execute(cls, **inputs: Any) -> io.NodeOutput:
        """Build one credential-reference-only R2 storage configuration value."""
        configuration_id = _hidden_unique_id(cls) or str(
            inputs.pop("unique_id", "r2-storage")
        )
        return io.NodeOutput(
            r2_storage_configuration_from_inputs(configuration_id, inputs)
        )


def _r2_storage_inputs() -> list[io.Input]:
    """Return non-secret workflow widgets shared by every R2 node instance."""
    return [
        io.String.Input(
            "configuration_name",
            default="r2-cache",
            tooltip="Workflow-local label for this storage backing.",
        ),
        io.String.Input(
            "account_id",
            default="",
            tooltip="Filled automatically after Cloudflare Login.",
        ),
        io.String.Input(
            "bucket",
            default="comfy-modal-cache",
            tooltip="Private R2 bucket created or reused during Login.",
        ),
        io.Combo.Input(
            "jurisdiction",
            options=["default", "eu", "fedramp", "us"],
            default="default",
            tooltip="R2 data jurisdiction and S3 endpoint family.",
        ),
        io.Combo.Input(
            "write_back_mode",
            options=["async", "off"],
        ...
        io.String.Input(
            "key_prefix",
            default="comfy-modal-cache/v1/blobs/sha256",
            advanced=True,
            tooltip="Immutable content-addressed object namespace.",
        ),
        io.String.Input(
            "credential_id",
            default="",
            advanced=True,
            tooltip=(
                "Random local keyring reference generated by the UI; it is not a "
                "credential."
            ),
        ),
    ]
...
def r2_storage_configuration_from_inputs(
    configuration_id: str,
    inputs: Mapping[str, Any],
) -> R2StorageBackingConfiguration:
    """Build one non-secret R2 storage configuration from queued widget inputs."""
    normalized_id = str(configuration_id).strip()
    return R2StorageBackingConfiguration(
        configuration_id=normalized_id,
        display_name=str(inputs.get("configuration_name") or "r2-cache").strip(),
        account_id=str(inputs.get("account_id") or "").strip(),
        bucket=str(inputs.get("bucket") or "").strip(),
        credential_id=str(inputs.get("credential_id") or normalized_id).strip(),
        jurisdiction=str(inputs.get("jurisdiction") or "default").strip().casefold(),
        key_prefix=str(
            inputs.get("key_prefix") or "comfy-modal-cache/v1/blobs/sha256"
        ).strip(),
        write_back_mode=str(inputs.get("write_back_mode") or "async")
        .strip()
        .casefold(),
    )
