...
if __package__:
    ...
    from .snapshot_lease import (
        _DEFAULT_LEASE_HEARTBEAT_STALE_SECONDS,
        _local_lease_owner_is_alive,
        _process_start_identity,
        _snapshot_lease as _generic_snapshot_lease,
    )
...
def _snapshot_lease(
    ...
    *,
    ...
    """Adapt generic lease wait messages to LLM staging progress events."""
    def report_wait(message: str) -> None:
        """Publish one generic lease wait through the staging callback."""
    ...
    with _generic_snapshot_lease(
        snapshot_path,
        progress_callback=report_wait if progress_callback is not None else None,
