...
The B300 runtime pins Torch 2.13.0, torchvision 0.28.0, Transformers 5.15.0, and vLLM 0.27.1. vLLM is installed only in CUDA 13.2 B300/B200+ images; established CUDA 12.8 GPU profiles retain their existing Torch 2.10 stack. The initial vLLM policy uses a 12 GiB KV cache, one request at a time, and eager execution to keep graph-capture allocations predictable beside ComfyUI. The worker sets `VLLM_USE_FLASHINFER_SAMPLER=0`, selecting vLLM's native sampler so its optional FlashInfer sampling kernel does not require an `nvcc` JIT compiler in the runtime image; this does not disable model-specific FlashInfer attention paths.
