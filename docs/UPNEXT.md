...
- Session-backed split proxies still do not participate in that local cache reuse. Replaying cached remote-session refs across prompt runs is unsafe, so the next caching improvement would need a real session-replay or rehydration design rather than more signature stripping.
