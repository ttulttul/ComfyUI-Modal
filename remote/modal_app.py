...
class ModalHourlyBillingStatus:
    ...
    def as_dict(self) -> dict[str, str | float | bool | None]:
        ...
        return {
            ...
            "app_cost_usd_before_credits": float(
                self.app_cost_usd_before_credits
            ),
...
def _completed_modal_billing_interval(
    ...
    if now.tzinfo is None:
        normalized_now = now.replace(tzinfo=timezone.utc)
    else:
    ...
    next_refresh_at = (
        interval_end
        + _MODAL_BILLING_INTERVAL
        + _MODAL_BILLING_COLLECTION_BUFFER
    )
...
def _prune_modal_hourly_billing_cache() -> None:
    ...
    oldest_keys = sorted(
        retained_keys,
        key=lambda cache_key: cache_key[2],
    )[:overflow]
...
def _fetch_modal_hourly_billing_synchronously(
    ...
    *,
    ...
    cache_environment_name = environment_name or "<workspace default>"
    cache_key = (cache_environment_name, app_name, interval_start)
