...
- Durable bridge records now have two direct-restore tiers: serialized payload restore for `CONDITIONING`, and single-node rehydration plans for self-contained `MODEL` producers. When a fresh worker cannot use the in-process bridge cache, it can rebuild just the standalone model-loader node instead of replaying the whole static producer phase.
