...
CPU memory snapshots are enabled by default. GPU memory snapshots are also enabled by default in current settings, but useful GPU snapshot work is limited to stable loader profiles derived from root literal model-loader nodes. Generic no-profile workers skip GPU snapshot prewarm because they do not provide the model-loaded cold-start win. Split phases assigned to the isolated LLM worker pool deliberately omit surrounding Comfy loader profiles, allowing them to reuse a clean LLM container without loading CLIP or diffusion models first.
