...
- Local ComfyUI output caching now works again for cache-safe remote proxies. For ordinary remote components without prompt-scoped remote-session state, the rewritten proxy payload no longer bakes `prompt_id` into `original_node_data`, so rerunning the same graph can reuse local cached outputs instead of reissuing the same remote execution.
