...
- Durable bridge records now have three direct-restore tiers: serialized payload restore for `CONDITIONING` and `LATENT`, single-node rehydration plans for self-contained `MODEL` producers, and trimmed non-sampler subgraph plans for linked `MODEL` producers such as loader-to-LoRA chains. When a fresh worker cannot use the in-process bridge cache, it can restore sampler latents directly or rebuild only the required model producer closure instead of replaying the whole sampler component.
