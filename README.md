...
It returns three strings: the clean final `response`, compact `metadata_json`, and a separate `reasoning` channel. The reasoning output is appended after the two original outputs so existing response and metadata links retain their indices. For thinking models, `max_new_tokens` covers reasoning and final-response tokens together; a generation that reaches the limit before its reasoning terminator produces an empty response and keeps all partial work in `reasoning`. Telemetry includes total output tokens, time to first token, live/final tokens per second, detected reasoning tokens and parser, cold/warm model status, media counts, GPU memory, the resident LLM profiles, and ComfyUI-managed models still resident after generation. The node's ordinary progress panel carries compact stages for profile/input preparation, CPU snapshot download, processor and weight loading, vLLM engine warmup, prefill, and generation. Known file, shard, and token totals use numeric bars; compilation and warmup use an indeterminate pulse. Prompt cancellation remains wired through ComfyUI's interruption hook without a separate inference server.

Modal LLM uses immutable schema-v2 model profiles. Checked-in entries in [`llm_profiles.json`](llm_profiles.json) remain available, while generated profiles content-address the exact Hugging Face revision, compatibility-policy version, backend, quantization, reasoning parser, context/media limits, expected VRAM, and remote-code policy. The compatibility registry currently selects Transformers for Muse-Glimmer and vLLM for Qwen3.5, including block-FP8 and NVIDIA ModelOpt NVFP4 checkpoints. Qwen3.5 profiles select the `qwen3` reasoning parser. Unknown architectures fail compatibility inspection before weight download.
...
Before any GPU call, the local dispatcher finds fixed LLM profiles in the remote prompt and calls the deployed CPU-only `ModelStager`. The stager downloads a missing immutable snapshot into `<volume>/llm_models/<profile>/<revision>`, writes a completion marker, commits the shared Modal Volume, and only then allows the selected GPU worker to start or reload that volume revision. A linked/dynamic `model_profile` is rejected because it cannot be staged before GPU allocation.

The warm worker holds a process-global, serialized LRU of Transformers and vLLM models. A cache hit reuses both processor and weights across node and workflow executions. Before a cold load, it asks ComfyUI to release idle managed models, evicts older resident LLMs if necessary, and enforces the requested free-VRAM reserve. This makes co-residency intentional while allowing ComfyUI's image/video loaders to use the rest of a large GPU. Both backends pass generated token IDs through one model-aware reasoning abstraction. It prefers an engine-native separated field when available and otherwise splits exact model boundary tokens before decoding each clean channel. Transformers currently returns generated IDs without a reasoning field. The resident vLLM path owns one persistent event-loop thread and one `AsyncLLM` per resident model; cumulative `generate()` outputs provide live token counts while preserving the same in-process co-residency and LRU behavior. vLLM workers use the native top-k/top-p sampler and explicit Triton full attention because FlashInfer's optional sampler and Blackwell TRT-LLM attention kernels otherwise attempt first-run `nvcc` JIT compilation, while FlashAttention 4 rejects Qwen3.5's used-sequence prefill at head dimension 256. Qwen's model-specific GDN kernel remains available.
