...
- Durable bridge records now have three direct-restore tiers: serialized payload restore for `CONDITIONING` and `LATENT`, and single-node rehydration plans for self-contained `MODEL` producers. When a fresh worker cannot use the in-process bridge cache, it can restore sampler latents directly or rebuild just the standalone model-loader node instead of replaying the whole static producer phase.
