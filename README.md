...
- That cache reuse is intentionally limited to proxies whose outputs are fully transportable. Split proxies or phase payloads that carry prompt-scoped remote-session refs still keep their prompt-scoped payload fields, because cached session refs cannot be replayed safely across prompt runs.
