...
- A frontend extension in [web/modal_toggle.js](/home/user/git/ComfyUI-Modal/web/modal_toggle.js) that injects a `Run on Modal` toggle into every non-internal node, draws live remote-state overlays on marked nodes, and submits queued prompts to `/modal/queue_prompt`.
- A backend route in [api_intercept.py](/home/user/git/ComfyUI-Modal/api_intercept.py) that reads the workflow snapshot from `extra_pnginfo.workflow`, recursively discovers Modal markers even inside nested subgraph workflow metadata, resolves those nested markers onto the actual queued prompt ids (including composed ids like `24:23` when ComfyUI expands subgraph internals), identifies connected remote-marked node components, syncs referenced assets, and rewrites each component into one signature-preserving Modal proxy node.
- A dynamic proxy registry in [modal_executor_node.py](/home/user/git/ComfyUI-Modal/modal_executor_node.py) that mirrors the exported output count and output types of each remote component so ComfyUI validation still succeeds after rewrite.
- A content-addressable sync engine in [sync_engine.py](/home/user/git/ComfyUI-Modal/sync_engine.py) that mirrors model files and a zipped `custom_nodes/` bundle into storage keyed by SHA256.
- A remote runtime skeleton in [remote/modal_app.py](/home/user/git/ComfyUI-Modal/remote/modal_app.py) that supports local fallback execution for tests and can call into Modal when the SDK is available.
- A stable Modal cloud entry in [comfyui_modal_sync_cloud.py](/home/user/git/ComfyUI-Modal/comfyui_modal_sync_cloud.py) that packages the local ComfyUI checkout plus the core ComfyUI Python runtime dependencies into the remote image.
...
2. The route loads the workflow metadata from `extra_data.extra_pnginfo.workflow`, including nested saved subgraph fragments.
...
- the backend route from [api_intercept.py](/home/user/git/ComfyUI-Modal/api_intercept.py)
- the internal proxy node definitions from [modal_executor_node.py](/home/user/git/ComfyUI-Modal/modal_executor_node.py)
- the frontend extension from [web/modal_toggle.js](/home/user/git/ComfyUI-Modal/web/modal_toggle.js)
...
The remote runtime skeleton lives in [remote/modal_app.py](/home/user/git/ComfyUI-Modal/remote/modal_app.py). The extension is structured for Modal, but you still need to supply a working Modal environment.
...
- remote mode will auto-deploy [comfyui_modal_sync_cloud.py](/home/user/git/ComfyUI-Modal/comfyui_modal_sync_cloud.py) on first use when the named app is missing, assuming Modal credentials are already configured in the ComfyUI environment
...
- the actual Modal cloud service now lives in a stable importable module, [comfyui_modal_sync_cloud.py](/home/user/git/ComfyUI-Modal/comfyui_modal_sync_cloud.py), because ComfyUI’s custom-node loader does not assign Modal-safe module names to node-pack directories
...
- Warm Modal containers now keep a loader-output cache for the heaviest built-in loaders (`UNETLoader`, `CLIPLoader`, and `VAELoader`), so repeated requests with the same resolved model inputs can reuse already-loaded runtime objects instead of loading those models from scratch every time
- Warm Modal containers now also reuse ComfyUI `PromptExecutor` instances per cache scope, so ordinary node outputs can stay cached across repeated remote runs the way they do in local ComfyUI instead of forcing every unchanged non-loader node to recompute
...
Avoid marking nodes remote if they depend on local-only process state or return objects that cannot be serialized by [serialization.py](/home/user/git/ComfyUI-Modal/serialization.py).
...
1. The request handler inspects the workflow snapshot in `extra_pnginfo.workflow`.
   Modal marker discovery now walks nested saved subgraph fragments too, and nested markers are resolved onto the actual queued prompt ids before rewrite runs. When ComfyUI exposes expanded subgraph internals as composed ids like `24:23`, those exact ids are targeted; otherwise the resolver falls back to the nearest ancestor prompt node.
...
The badge also now refreshes its derived phase when the browser tab regains visibility or focus, so background-tab throttling does not leave it stuck on an old stage after you switch back.
...
- The project includes a minimal [pyproject.toml](/home/user/git/ComfyUI-Modal/pyproject.toml) so pytest discovery is consistent under `uv`.
- [modal_test_workflow.json](/home/user/git/ComfyUI-Modal/modal_test_workflow.json) is checked in as the first captured real workflow that successfully produced output through the Modal path. It is treated as a smoke-test artifact, not a pristine editor-authored workflow.
