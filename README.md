...
- Local ComfyUI output caching now works again for cache-safe remote proxies. The rewritten proxy payload strips run-scoped queue metadata like `prompt_id`, `extra_data`, and volume-reload bookkeeping out of `original_node_data`, so rerunning the same graph can reuse local cached outputs instead of reissuing the same remote execution just to discover inner Modal Dict hits.
...
- Outer implicit Modal fan-out is deliberately disabled when a split boundary input lands on an `INPUT_IS_LIST` node. Those nodes need the full list in one call, so Modal-Sync now executes the remote subgraph once unchanged and lets ComfyUI handle any downstream over-list behavior internally instead of replaying scalar sibling work per item.
...
- Session-backed split proxies now participate in local ComfyUI cache reuse too. Their rewritten proxy payload strips run-scoped `prompt_id`, `remote_session`, and `clear_remote_session` from the cache surface and rehydrates those fields only if the proxy actually executes.
- That rehydration no longer depends only on ComfyUI passing the hidden `unique_id` back to the proxy. Sanitized cache-friendly payloads also carry a stable internal context id so session-backed executions can still restore their full runtime payload when that hidden field is omitted on a real execution path.
- Split-phase remote distributed caching now preserves stable provenance for non-transportable boundary-fed inputs. When a later remote phase receives live runtime objects like `ModelPatcher` or `CONDITIONING` from a boundary injection, the persisted Modal Dict cache key uses a structural fingerprint of the original upstream prompt subgraph instead of the live Python object identity, so repeated workflow runs can reuse sampler-adjacent cached nodes across containers.
...
- Session-backed bridge outputs are durable replay refs now, not raw prompt-scoped session refs. The live remote-only object still stays in the in-memory session store for the fast path, but a separate Modal `Dict` stores replay metadata for the producing phase so a later split proxy can rehydrate that value into its current session after Modal scales down or reroutes the work to a new container.
- Every synthesized static or mapped phase payload now preserves `remote_session` as well. Without that, a replayed producer phase can get all the way to boundary-output export before failing with `Session-backed boundary outputs require payload.remote_session.`
- Frontend mapped-progress lane ids are now scoped by component representative, not just by numeric lane id. That prevents two different remote components from both using lane `0` and stealing each other's purple progress bars in the UI, and streamed completion can now still consume the proxy's final native completion event so single-node remote components do not stay purple after they finish.
