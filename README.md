...
- Preview taps also absorb required non-transportable upstream dependencies. For example, if an absorbed `VAEDecode` preview uses a `VAELoader`, the loader joins the same remote component so the proxy never tries to transport a `VAE` value across the local/remote boundary.
