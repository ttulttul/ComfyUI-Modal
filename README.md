...
Subrosa support uses the relay-managed worker service. A **Subrosa Configuration** declares a `ws://`/`wss://` relay base URL, pool, concurrency limit, and advanced `credential_id` widget. The keyring reference defaults to the stable, human-readable `subrosa-default`, so one saved token can serve every workflow; changing it intentionally selects a different local token. Use **Click to Authenticate** to open the trusted relay portal, authenticate with a passkey or recovery code, mint a new `srk_` extension token, validate it locally, and save it in the operating-system keyring. Login and token validation accept HTTPS `subrosa.red` services (plus loopback development servers), preventing a workflow-supplied relay from receiving a saved service token. The token is never stored in the workflow or queue payload. The frontend explicitly materializes the effective `credential_id` in the queued prompt even when ComfyUI omits a default-valued advanced widget, ensuring the node status and execution preflight validate the same keyring entry. The button checks saved-token validity through `whoami`; queue interception repeats that check before prompt rewriting or ComfyUI node execution, and the relay client retains a final defensive check before opening a billable job. A missing or rejected token durably changes the responsible node's button to **Click to Authenticate**, highlights that node in red, and shows the same concise authentication instruction instead of attributing the failure to a downstream workflow node. The scheduler advertises pool-specific synthetic hardware: `mock-4090` has 24 GiB VRAM and `mock-B300` has 288 GiB VRAM, while unknown pools conservatively fall back to 24 GiB. Only the B300 VRAM figure is authoritative; its CPU count and host RAM are plausible planning values, and memory bandwidth and bus width are not represented by the scheduler capability model. The client waits for a claimed worker, transports the existing CRMTRPC1 request/input stream in 512 KiB lane-0 messages, streams progress back to the canvas, reports relay settlement and centicredits, and sends cancellation over lane 1 on the same socket.

Subrosa asset preparation reuses the same content-addressed model and custom-node discovery as direct Modal execution. Before queueing, the extension asks the authenticated Subrosa asset service which SHA-256 objects are missing, uploads only cache misses through short-lived exact-object R2 capabilities, uses multipart uploads for files above R2's single-PUT ceiling, and creates an immutable environment manifest. The queued workflow carries only that opaque manifest ID and `/storage` paths. The service stages verified objects and custom-node Python requirements on CPU, then mounts only the requesting account's cached Volume subpath into a GPU worker reserved to that account and immutable manifest. All split proxy invocations in the queued workflow share the manifest and worker, preserving live remote-session values; a different manifest cannot use that kernel. Unchanged models, archives, and dependency environments are reused on later workflows without exposing Subrosa storage credentials or another account's files. A local macOS or Windows virtual environment is never copied to Linux; custom-node `requirements.txt` entries are reconstructed in a Linux site-packages layer keyed by their environment fingerprint. Local paths, environment substitutions, and credential-bearing dependency URLs are rejected. New system packages not present in the pinned worker image still require a worker-image update.

Login writes the token from the local ComfyUI backend, so macOS must have an
unlocked GUI login keychain. The following remains available as a manual
recovery path and prompts without placing the token in shell history:
...
Direct Vast SSH operations tolerate bounded connection-level closures, resets, and handshake interruptions with jittered exponential retries. Worker-readiness SSH probes start one second apart and multiply the interval by 1.5 after every unsuccessful attempt, with each failure and next delay streamed to the workflow UI. Vast's direct public port mapping remains preferred, but the controller also retains the provider's `ssh*.vast.ai` proxy endpoint; if the direct readiness probe fails while the proxy succeeds, the proxy becomes the persisted endpoint for staging, execution, transfers, and watchdog updates. Authentication rejection and deterministic remote-command failures remain immediate. Streamed uploads reopen the local source from byte zero on every attempt, and the remote atomic writer verifies the expected byte count before publishing, so a killed connection cannot leave a truncated file at the content-addressed destination.

Cancelling a workflow also cancels queue-time remote preparation. Intentional cancellation remains an interrupted workflow while Vast capacity is being requested or initialized; it is not recast as a provider-acquisition validation failure. File hashing checks the prompt-scoped cancellation signal between chunks, and active Vast SSH uploads or Hugging Face downloads terminate their local SSH process instead of continuing behind ComfyUI's cancelling status. Vast Hugging Face acquisition runs from a small content-addressed tool bundle uploaded by the controller, so an older worker image cannot force a multi-gigabyte fallback upload merely because it lacks the newest materializer module. The launch environment does not overwrite the runtime fingerprint baked into the published worker image. The Vast fingerprint hashes only content that ships in the image: pinned dependency inputs, the extension source digest, and the curated ComfyUI source digest. Modal-only settings such as GPU selection, app and dictionary names, and container counts do not invalidate a functionally identical Vast image. Planning captures this fingerprint once and carries it, together with the selected worker digest, through lease reuse and every invocation instead of hashing the live source tree again during dispatch. Before requesting or reusing billable capacity, the controller reads the public image's OCI manifest and small config blob, selects its `linux/amd64` variant, and compares the baked runtime-fingerprint label without pulling any filesystem layers. The selected platform manifest is resolved to an immutable digest; lease reuse requires that exact digest, and every Vast instance launch uses it rather than a mutable tag. A mismatch is never accepted solely because the protocol number matches: the controller runs the image-build script once for the current fingerprint with the exact Python interpreter hosting ComfyUI, streams build and push output through ComfyUI's remote-preparation progress, adopts the returned immutable digest for the current process, and only then requests capacity. Worker-side validation remains as defense in depth; if it still finds drift, the stale lease is terminated before one replacement attempt. Invoking `sys.executable` directly avoids assumptions about virtualenv location or activation and prevents an obsolete copied `.venv` from hijacking the build. The build subprocess must report the same source fingerprint captured by the parent; if image content changed afterward, publication stops before Docker work or another rental and asks for a restart. Concurrent workflows share the completed publication. This automatic path requires a running Docker service and registry push credentials on the ComfyUI host; if any prerequisite or publication step fails, the workflow stops with the exact manual recovery command instead of executing on stale source. Stale registry entries are discarded whether Vast reports a missing contract through HTTP 404 or its v0 `instances: null` response.
...
Active remote nodes show a small progress panel with a Modal, SSH terminal, or Vast.ai icon beside every bar, per-bar `it/s` rates, human-readable transferred/total byte counts and `B/s` through `TB/s` rates for large input and output transfers, and `tok/s`, token counts, and TTFT for LLM nodes. Modal uploads pulse until the SDK reports its first remote event; large Modal results travel through the durable volume so their download has exact byte progress. SSH and Vast input/output frames stream through their relays in bounded chunks and report exact byte progress rather than buffering the complete payload first. During execution, every Modal, SSH, and Vast worker samples its process tree's CPU RAM and the memory consumed across all visible CUDA devices twice per second. The Configurator reports current and peak GPU/CPU memory for each environment, aggregates concurrently active execution sources, and leaves the peak readout visible when the workflow finishes. LLM preparation is reported on the actual LLM node—not the remote component's representative node—with distinct metadata inspection, snapshot lookup, concurrent-download wait, download preparation, transfer, cached, and ready states. When a prompt is scheduled across more than one remote environment, the panel also names where each component is running — the Modal container id or the SSH host name. Preview images and UI payloads from remote nodes stream back into the local session while the component is still running, and progress state is reconciled against queue history when the browser regains focus so cancelled prompts do not leave stale bars behind.
...
New local or Hugging Face transfers are written back to R2 through a deduplicated two-worker idle queue. Cache work never delays prompt preparation: it waits until no remote workflow is preparing, queued, or running, and a newly submitted workflow preempts and requeues any active cache transfer. Indexed worker-storage hits are also checked for write-back, which lets enabling R2 later or retrying after a transient failure backfill existing assets without another controller-to-worker upload. Indexed custom-node traversal and packaging use the same idle queue rather than delaying the current workflow. Select `off` for read-only use. Legacy `sync` settings are accepted but migrate to idle background behavior. The cache also covers custom-node archives, manifests, and package-owned model assets. A custom-node source-tree digest still selects its reusable archive path, while the archive's actual byte digest is the R2 object identity. Modal execution does not use R2 because its native persistent volume already supplies the fast shared layer.

Permanent R2 access credentials remain only in the local ComfyUI controller. Workers receive expiring, exact-object S3-compatible GET/PUT URLs through protected process standard input; URLs do not enter workflow data, Docker/SSH command arguments, launch environments, or logs. Before any asset transfer, the controller validates its bucket access and each Vast.ai or self-hosted SSH environment performs a read-only ranged GET against a random absent sentinel. The expected `404` proves that the worker's actual egress can use the token; a `403 AccessDenied` fails preparation immediately with a client-IP-filtering hint instead of discovering the policy problem during multi-gigabyte transfers. Downloads and uploads are SHA-256 and size verified rather than trusting an S3 ETag. Self-hosted SSH write-back ensures its fingerprinted runtime image exists before launching the Docker materializer. Transfer failures expose only a fixed category, numeric HTTP status, and bounded R2 error code; exception text and presigned request details remain suppressed. Files above the configured single-PUT threshold use multipart upload, which also avoids buffering model files in controller memory.
...
| `COMFY_MODAL_R2_WRITE_BACK` | `async` | Cache population mode: idle-background `async` or read-only `off`; legacy `sync` values migrate to `async`. |
...
| `COMFY_MODAL_VAST_IMAGE` | unset | Public worker image reference required for Vast execution. Tags are accepted for registry lookup, but every instance is launched with the resolved immutable `linux/amd64` digest. |
...
The prompt interception layer is organized as a compatibility aggregator over focused modules: planning values, graph analysis, component planning, prompt rewriting, execution scheduling, queue lifecycle, diagnostics, and HTTP route domains each have a dedicated owner. Route handlers share an immutable `RouteContext`, while `api_intercept.py` retains only registration and compatibility re-exports. Host-side remote execution follows the same ownership model: deployment loading, runtime compatibility checks, and auto-deploy state live in `remote/modal_deployment.py`; proactive warmup, snapshot profiles, and local-gap keepalive state live in `remote/modal_warmup.py`; mapped or implicit-batch lane scheduling lives in `remote/mapped_execution.py`; and CPU ModelStager state lives in `remote/modal_llm_profile_staging.py`. Adjacent invocation and staging callbacks are injected by the `remote/modal_app.py` orchestrator. Blocking Modal executors are constructed lazily on first use, and importing the host orchestrator no longer constructs a vestigial Modal app or volume. Cloud helpers receive tail-created Modal dictionaries and volumes through `cloud_runtime_context.py`, active request cleanup through `cloud_execution_control.py`, and timestamped phase reporting through `cloud_runtime_logging.py`, allowing focused cloud subsystems to move without reaching back into entry-module globals; pre-deploy app collision checks live in `cloud_app_guard.py`, image filters and worker construction policy live in `cloud_image_env.py`, durable invocation replay and live-canary coordination live in `cloud_durable_invocation.py`, session-bridge storage and replay live in `cloud_session_bridge.py`, headless ComfyUI server compatibility plus progress tracing live in `cloud_prompt_server_shims.py`, ComfyUI initialization, custom-node loading, asset path translation, compatibility patches, and warm loader caches live in `cloud_comfy_bootstrap.py`, distributed node-output key construction, serialization, restore, and write-back live in `cloud_node_output_cache.py`, prompt schema validation and normalization live in `cloud_prompt_validation.py`, reusable executor management and local subgraph execution live in `cloud_prompt_execution.py`, mapped/phased cloud execution lives in `cloud_mapped_execution.py`, bounded progress/result delivery plus abandoned invocation cleanup live in `cloud_streaming.py`, mounted-volume visibility and reload recovery live in `cloud_volume_reload.py`, and snapshot, loader, LLM, and request prewarming live in `cloud_prewarm.py`. The stable `comfyui_modal_sync_cloud.py` entrypoint imports only the hooks and operations it executes; legacy reads of extracted private names resolve lazily to their owning module. Root modules support both package imports used by ComfyUI and flat imports used by the Modal container; `tests/test_import_shapes.py` guards both layouts and the stable cloud surface. The integration tests mirror those ownership boundaries in focused files and keep shared doubles and builders in non-collected `*_test_support.py` modules.
...
The stable `api_intercept.py` entrypoint now limits itself to compatibility exports, route context construction, and route registration. Queue-time selection, capacity planning coordination, bounded per-environment asset preparation, and proxy rewrite orchestration live in `prompt_interception.py`.

The host-side Modal orchestrator delegates prompt cancellation state and propagation to `remote/modal_interrupts.py`, and streamed remote event decoding plus local UI forwarding to `remote/payload_stream.py`. `remote/modal_app.py` retains invocation, recovery, materialization, and public entrypoint coordination.
