...
It returns three strings: the clean final `response`, compact `metadata_json`, and a separate `reasoning` channel. The reasoning output is appended after the two original outputs so existing response and metadata links retain their indices. For thinking models, `max_new_tokens` covers reasoning and final-response tokens together. The MLX backend caps thinking at half that budget so it retains room for a final answer; the remote backend retains its existing model-native thinking behavior. Telemetry includes execution target, device, total output tokens, time to first token, live/final tokens per second, detected reasoning tokens and parser, cold/warm model status, media counts, available memory, resident LLM profiles, and ComfyUI-managed models still resident after generation. The node's ordinary progress panel carries compact stages for profile/input preparation, snapshot download, processor and weight loading, engine warmup where applicable, prefill, and generation. Prompt cancellation remains wired through ComfyUI's interruption hook without a separate inference server.
