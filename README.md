...
Permanent R2 access credentials remain only in the local ComfyUI controller. Workers receive expiring, exact-object S3-compatible GET/PUT URLs through protected process standard input; URLs do not enter workflow data, Docker/SSH command arguments, launch environments, or logs. Downloads and uploads are SHA-256 and size verified rather than trusting an S3 ETag. Self-hosted SSH write-back ensures its fingerprinted runtime image exists before launching the Docker materializer. Transfer failures expose only a fixed category and, for HTTP failures, the numeric status code; exception text and presigned request details remain suppressed. Files above the configured single-PUT threshold use multipart upload, which also avoids buffering model files in controller memory.
