...
- Remote session bridge reuse is keyed by durable bridge id as well as node output coordinates, so mapped outputs from the same node slot, such as per-prompt `CONDITIONING` values, do not collapse to the first resolved item.
