...
CPU memory snapshots are enabled by default. GPU memory snapshots are also enabled by default in current settings, but useful GPU snapshot work is limited to stable loader profiles derived from root literal model-loader nodes. Each loader profile records direct-start and GPU-snapshot warmup timings in its snapshot-profile `Dict`; the first two warmups sample both modes and later starts select the lower median rather than assuming snapshots are universally faster. Generic no-profile and isolated LLM workers use direct startup. Independent root CLIP, UNET, and checkpoint loader plans run concurrently under a small worker limit. A real dispatch joins an exact matching speculative warmup—app, GPU, affinity, loader profile, and startup mode—instead of racing a duplicate cold container. After an automatic deployment, the first important profile is seeded and joined before workflow execution continues.
