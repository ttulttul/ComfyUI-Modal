...
Modal hardware is fixed at deploy time. Choose the target from `Modal` → `GPU`; the next remote run looks up or automatically builds the persistent Modal app dedicated to that GPU. The default `A100` target retains the configured base app name for compatibility, while other targets use a readable suffix such as `-gpu-b300`. GPU-specific app identity prevents a B300 request from contacting an existing A100 class merely to inspect its runtime fingerprint, which would otherwise provision the wrong GPU before the comparison could finish. The GPU apps continue to share the configured asset Volume and durable cache names. If you upgrade this node pack and expect changed remote behavior, redeploy the affected GPU app once so it picks up the new code and class options.
