...
- Modal volume metadata calls are now intentionally capped and share one exponential rate-limit backoff window per backend instance. That keeps parallel custom-node sync from turning into a `VolumeListFiles` burst storm on newer Modal accounts with tighter metadata limits.
