...
- The Modal entry modules keep real type annotations on `@app.cls` class parameters such as `session_affinity_key`. Postponed annotations in those files can break Modal class construction at import time because `modal.parameter(...)` validation expects a concrete Python type, not the string form of one.
