...
- The Modal entry modules keep real type annotations on `@app.cls` class parameters such as `worker_affinity_key`. Postponed annotations in those files can break Modal class construction at import time because `modal.parameter(...)` validation expects a concrete Python type, not the string form of one.
- Remote worker routing now uses reusable pool-scoped affinity keys like `worker-pool:slot:0` instead of prompt UUIDs. Prompt-scoped `remote_session.session_id` still isolates live bridge data, but worker affinity is now stable across prompts so warm containers can be reused between adjacent runs.
