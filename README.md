...
The curated `huihui-qwen3.8-27b-abliterated-q2-k-gguf` profile pins `huihui-ai/Huihui-Qwen3.8-27B-abliterated-GGUF` to one 10.12 GiB `Q2_K` file rather than downloading every quant in that repository. It stages separately pinned tokenizer metadata from the source checkpoint, runs text/file requests through the private loopback llama.cpp server, and uses an 8K context plus Q8 KV cache under a conservative 14.5 GiB VRAM floor. Set the node's `reserve_free_vram_gb` low enough for the selected SSH GPU; the profile remains text-only because it intentionally omits the vision projector.
